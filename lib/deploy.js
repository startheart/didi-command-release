...
function prepareDeploy(dest, file, callback) {
    if(file.release) {
        ...
        if(file.isText() && content.length) {
            ...
            if (name && file.isHtmlLike) {
                // quyatong
                content = didi.insertOmega(name, content, file, dest.opt);
            }

            //webzhangnan
            content = didi.insertPhoneLogScript(content, file, dest.opt);
            content = didi.insertWeinreScript(content, file, dest.opt);
            //startheart
            content = didi.insertDidiConsoleScript(content, file, dest.opt);
