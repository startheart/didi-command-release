...
function get_didi_console_source(){
    if(didi_console_source === undefined){
        var didi_console_filepath = require.resolve('didi-console');
        didi_console_source = fs.readFileSync(didi_console_filepath, 'utf8');
    }
    return didi_console_source;
}
exports.insertDidiConsoleScript = function(content , file, opt){

    if(file.isHtmlLike){
        if(opt.didiConsole){
            var id = 'didi_console_'+ Date.now();
            var code = [];
            code[code.length] = '<script type="text/javascript" charset="utf-8" id="' + id +'" > ';
            code[code.length] = get_didi_console_source();
            code[code.length] = ';!function(){\n var didiConsoleEl = document.getElementById("' + id + '");';
            code[code.length] = "didiConsoleEl.parentNode.removeChild(didiConsoleEl);";
            code[code.length] = '}();</script>';
            code = code.join('\r\n');
